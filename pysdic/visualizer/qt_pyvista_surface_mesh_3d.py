...
from ..geometry import SurfaceMesh3D, IntegrationPoints

class QtPyvistaSurfaceMesh3D(QtWidgets.QMainWindow):
    r"""
    QT application to visualize a :class:`pysdic.geometry.SurfaceMesh3D` with PyVista.

    The windows contains a ComboBox to select the property to visualize.

    - The properties stored into the :attr:`pysdic.geometry.SurfaceMesh3D._vertices_properties`.
    ...
    def __init__(self, 
        ...
        super().__init__()
        self.setWindowTitle("Visualisation SurfaceMesh3D with pyvista")
        ...
        # ----- Check the inputs -------
        if not isinstance(self.mesh, SurfaceMesh3D):
            raise TypeError("mesh must be an instance of SurfaceMesh3D.")
        ...
        celltypes = numpy.full(n_cells, mesh.vtk_cell_type, dtype=numpy.uint8)

        self.pv_mesh_property = pyvista.UnstructuredGrid(cells, celltypes, mesh.vertices.points)
        ...
        if self.textures is not None:
            ...
            self.pv_mesh_texture = pyvista.UnstructuredGrid(cells, celltypes, fictive_vertices)
            self.pv_mesh_texture.active_texture_coordinates = numpy.zeros((fictive_vertices.shape[0], 2), dtype=numpy.float64)
            uvmap = mesh.elements_uvmap  # shape (M, 6)
            for i in range(mesh.n_vertices_per_element):
                self.pv_mesh_texture.active_texture_coordinates[i::mesh.n_vertices_per_element, 0] = uvmap[:, 2 * i]      # u_i
                self.pv_mesh_texture.active_texture_coordinates[i::mesh.n_vertices_per_element, 1] = uvmap[:, 2 * i + 1]  # v_i
...
def visualize_qt_pyvista_surface_mesh_3d(
    mesh: SurfaceMesh3D, 
    property_arrays: Optional[Dict[str, numpy.ndarray]] = None,
    textures: Optional[Dict[str, numpy.ndarray]] = None,
    integration_points: Optional[Dict[str, IntegrationPoints]] = None,
    ...
    """
    Launch the QT application to visualize a :class:`pysdic.geometry.SurfaceMesh3D` with ``pyvista`` and ``pyqt5``.
    ...
    .. code-block:: python   
        ...
        visualize_qt_pyvista_surface_mesh_3d(
            mesh=surface_mesh,
    ...
    .. figure:: ../../../pysdic/resources/qt_pyvista_surface_mesh_3d_example_geometry.png
        :align: center
        :width: 500px
        :alt: QT PyVista Surface Mesh 3D Example (Geometry Mode)

    .. figure:: ../../../pysdic/resources/qt_pyvista_surface_mesh_3d_example_property.png
        :align: center
        :width: 500px
        :alt: QT PyVista Surface Mesh 3D Example (Property Mode)

    .. figure:: ../../../pysdic/resources/qt_pyvista_surface_mesh_3d_example_texture.png
        :align: center
        :width: 500px
        :alt: QT PyVista Surface Mesh 3D Example (Texture Mode)

    .. figure:: ../../../pysdic/resources/qt_pyvista_surface_mesh_3d_example_integration_points.png
        :align: center
        :width: 500px
        :alt: QT PyVista Surface Mesh 3D Example (integration points Mode)    

    """
    app = QtWidgets.QApplication([])
    window = QtPyvistaSurfaceMesh3D(
        mesh=mesh,
