...
class Mesh(ABC):
    ...
    .. seealso::
        ...
        - :doc:`/geometry/shape_functions` for more information on shape functions.

    The subclasses must define the following class attributes:

    - ``_expected_N_vpe`` : :class:`int`
    - ``_expected_K`` : :class:`int`
    - ``_meshio_cell_type`` : :class:`str`
    - ``_shape_functions_meth`` : :class:`Callable`
    ...
    _expected_N_vpe: int = None  # Expected number of vertices per element
    _expected_K: int = None  # Expected number of natural coordinates (topological dimensions)
    _meshio_cell_type: str = None  # Expected meshio cell type
    _shape_functions_meth: Callable = None  # Expected shape functions method
    ...
    def __init__(self, vertices: PointCloud, connectivity: numpy.ndarray, vertices_properties: Optional[Dict] = None, elements_properties: Optional[Dict] = None, internal_bypass: bool = False) -> None:
        ...
        if self._expected_K is not None and not vertices.n_dimensions >= self._expected_K:
            raise ValueError(f"Vertices must have embedding dimension greater or equal to {self._expected_K}, got {vertices.n_dimensions}.")
        if self._expected_N_vpe is not None and not connectivity.shape[1] == self._expected_N_vpe:
            raise ValueError(f"Connectivity must have {self._expected_N_vpe} columns, got {connectivity.shape[1]}.")
    ...
    def _internal_check_vertices(self) -> None:
        ...
        if self._expected_K is not None and not self._vertices.n_dimensions >= self._expected_K:
            raise ValueError(f"Vertices must have embedding dimension greater or equal to {self._expected_K}, got {self._vertices.n_dimensions}.")
    ...
    def _internal_check_connectivity(self) -> None:
        ...
        if self._expected_N_vpe is not None and not self._connectivity.shape[1] == self._expected_N_vpe:
            raise ValueError(f"Connectivity must have {self._expected_N_vpe} columns, got {self._connectivity.shape[1]}.")
    ...
    def _get_corresponding_vtk_cell_type(self) -> Optional[str]:
        r"""
        Internal method to get the corresponding VTK cell type for the meshio cell type.

        Returns
        -------
        Optional[:class:`str`]
            The corresponding VTK cell type, or None if meshio_cell_type is None.
        """
        ...
        return meshio_to_vtk.get(self._meshio_cell_type, None)
    ...
    # =======================
    @classmethod
    def from_meshio(cls, mesh: meshio.Mesh, load_properties: bool = True) -> Mesh:
        r"""
        ...
        shape_functions_meth : Optional[Callable], optional
            A callable to compute the shape functions at given natural coordinates, by default None.
        ...
        Lets create a mesh using :class:`meshio.Mesh` and convert it to a :class:`Triangle3Mesh` instance.
        ...
        Create a :class:`Triangle3Mesh` instance from the :class:`meshio.Mesh` object.

        .. code-block:: python

            mesh3d = Triangle3Mesh.from_meshio(mesh)
        ...
        if cls._expected_K is not None and not mesh.points.shape[1] >= cls._expected_K:
            raise ValueError(f"mesh.points must have embedding dimension greater or equal to {cls._expected_K}, got {mesh.points.shape[1]}.")
        ...
        if cls._expected_N_vpe is not None and not mesh.cells[0].data.shape[1] == cls._expected_N_vpe:
            raise ValueError(f"mesh.cells[0].data must have {cls._expected_N_vpe} columns, got {mesh.cells[0].data.shape[1]}.")
        if cls._meshio_cell_type is not None and not mesh.cells[0].type == cls._meshio_cell_type:
            raise ValueError(f"mesh.cells[0].type must be '{cls._meshio_cell_type}', got '{mesh.cells[0].type}'.")
    ...
    def to_meshio(self, save_properties: bool = True) -> meshio.Mesh:
        ...
        Examples
        --------
        Lets consider the subclass :class:`Triangle3Mesh` of Mesh.

        Create a simple :class:`Triangle3Mesh` instance.

        .. code-block:: python

            import numpy as np
            from pysdic.geometry import Triangle3Mesh, PointCloud

            points = np.array([[0, 0, 0], [1, 0, 0], [0, 1, 0], [0, 0, 1]])
            connectivity = np.array([[0, 1, 2], [0, 1, 3], [0, 2, 3], [1, 2, 3]])

            mesh3d = Triangle3Mesh(PointCloud.from_array(points), connectivity)

        Convert the :class:`Triangle3Mesh` instance to a :class:`meshio.Mesh` object.
        ...
        cells = [meshio.CellBlock(self._meshio_cell_type, data=self.connectivity)]
    ...
    @classmethod
    def from_vtk(cls, filename: str, load_properties: bool = True) -> Mesh:
        r"""
        ...
        Create a :class:`Triangle3Mesh` instance from the VTK file.

        .. code-block:: python

            mesh3d = Triangle3Mesh.from_vtk("simple_mesh.vtk")
            print(mesh3d.vertices)
            # Output: PointCloud with 4 points [[0, 0, 0], [1, 0, 0], [0, 1, 0], [0, 0, 1]]      
    ...
    def to_vtk(self, filename: str, save_properties: bool = True) -> None:
        r"""
        Write the Mesh instance to a VTK file. (Only for 3D embedding dimension meshes)
        ...
        Save the :class:`Triangle3Mesh` instance to a VTK file.
    ...
    def vertices(self) -> PointCloud:
        ...
        .. code-block:: python
            ...
            import numpy as np
            from pysdic.geometry import Mesh, PointCloud
    ...
    def n_topological_dimensions(self) -> int:
        ...
        if self._expected_K is None:
            raise NotImplementedError("Subclasses must implement n_topological_dimensions property.")
        return self._expected_K
    ...
    def get_vertices_property(self, key: str) -> Optional[numpy.ndarray]:
        ...
        Create a simple :class:`Triangle3Mesh` instance with a vertex property.
    ...
    def get_elements_property(self, key: str) -> Optional[numpy.ndarray]:
        ...
        Create a simple :class:`Triangle3Mesh` instance with an element property.
    ...
    def is_empty(self) -> bool:
        ...
        Create an empty :class:`Triangle3Mesh` instance.

        .. code-block:: python

            from pysdic.geometry import Triangle3Mesh

            empty_mesh3d = Triangle3Mesh.from_empty()
            print(empty_mesh3d.is_empty())
            # Output: True

        Create a non-empty :class:`Triangle3Mesh` instance.
    ...
    def copy(self) -> Mesh:
        ...
        .. note::
            ...
            This method creates a new Mesh instance with copies of the vertices, connectivity, and properties with the class method :func:`from_mesh`.
        ...
        return self.__class__(vertices, connectivity, new_vertices_properties, new_elements_properties)
    ...
    # =======================
    @abstractmethod
    def shape_functions(self, *args, **kwargs) -> Tuple[numpy.ndarray, Optional[numpy.ndarray]]:
        r"""
        Method to compute the shape functions and their first derivatives for the mesh elements.
        
        .. seealso::

            - :doc:`../geometry_functions/shape_functions` for more information on shape functions.
        
        """
        raise NotImplementedError("shape_functions method must be implemented in subclasses of Mesh.")
...
class SurfaceMesh(Mesh):
    r"""
    Abstract Base Class for 2D topological Meshes (:math:`K=2`).

    This class extends the :class:`Mesh` abstract base class to represent 2D meshes.
    It serves as a base for specific surfacique mesh implementations.

    This mesh is characterized by having 2D elements (e.g., triangles, quadrilaterals) defined in a space of dimension :math:`E >= 2`.

    The coordinates of a point into the mesh can be accessed by the natural coordinates
    in the reference element. The number of natural coordinates :math:`(\xi, \eta)` with a topological dimension of :math:`K=2`.
    ...
    _expected_K: int = 2  # Topological dimension for 2D meshes
    ...
    def visualize(
        ...
        r"""
        Visualize the surface mesh using PyVista (only for :math:`E \leq 3` meshes).
        ...
        .. figure:: /_static/meshes/linear_triangle_mesh_3d_visualize_example.png
           :width: 600
           :align: center
        ...
        vtk_cell_type = mesh._get_corresponding_vtk_cell_type()
        if vtk_cell_type is None:
            raise NotImplementedError(
                f"Visualization is not supported for mesh elements of type {self.__class__.__name__}."
            )
    ...
    def visualize_vertices_property(
        ...
        .. figure:: /_static/meshes/linear_triangle_mesh_3d_visualize_vertices_property_example.png
           :width: 600
           :align: center
        ...
        if vtk_cell_type is None:
            raise NotImplementedError(
                f"Visualization is not supported for mesh elements of type {mesh.__class__.__name__}."
            )
        cells = numpy.hstack([numpy.full((n_cells, 1), mesh.n_vertices_per_element), mesh.connectivity]).astype(numpy.int64).ravel()
        celltypes = numpy.full(n_cells, vtk_cell_type, dtype=numpy.uint8)
    ...
    def visualize_texture(
        ...
        .. figure:: /_static/meshes/linear_triangle_mesh_3d_visualize_texture_example.png
            :width: 600
            :align: center
